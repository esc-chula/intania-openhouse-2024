...
export default function InnovationCard({
  ...
  return (
    <div
      className="flex flex-col justify-between gap-4 rounded-[20px] bg-button-solid px-5 py-6 shadow-button-solid"
      onClick={() =>
        router.push(
          `/CUIIC/${innovation.name.slice(0, 2)}/${innovation.name.slice(0, 4)}/`,
        )
      }
    >
      <div className="relative h-44">
        <Image
          src={`https://drive.google.com/uc?export=view&id=${innovation.image.split("id=")[1]}`}
