...
export const POST = async (req: NextRequest) => {
  ...
  if (tourData.users.length >= tourData.maxUser) {
    return NextResponse.json(
      { message: "Tour is fully reserved" },
      { status: 400 },
    );
  }
...
export const DELETE = async (req: NextRequest) => {
  ...
  if (!tourData.users.includes(parsedData.userId)) {
    return NextResponse.json(
      { message: "User has not reserve the tour" },
      { status: 400 },
    );
  }

  const updatedTour = await updateDocument("tours", parsedData.tourId, {
    users: tourData.users.filter((userId) => userId !== parsedData.userId),
  });

  const updatedUser = await updateDocument("users", parsedData.userId, {
    tours: userData.tours.filter((tourId) => {
